...
import { CurrentTyreSet } from '../components/vehicles/CurrentTyreSet';
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-4">
                      <span className="inline-flex rounded-lg bg-[#202124] px-2.5 py-1 text-[11px] font-black tracking-wide text-white">{vehicle.registration}</span>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-4 py-4 text-right">
                      <button
                        onClick={(event) => { event.stopPropagation(); setSelectedVehicleId(vehicle.id); }}
          ...
          <div className="fixed inset-0 z-50 bg-black/45 backdrop-blur-[2px]">
            ...
            <aside className="absolute inset-y-0 right-0 z-10 w-full max-w-[560px] overflow-y-auto bg-[#F7F6F2] shadow-2xl">
              ...
              <div className="space-y-5 p-5 sm:p-6">
                ...
                <CurrentTyreSet activeTyres={selectedVehicle.activeTyres} />

                <section className="rounded-[22px] bg-white p-5 ring-1 ring-black/[0.045]">
                  <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-zinc-400">Next attention</p>
                  <div className="mt-3 flex items-center justify-between gap-4">
                    <div>
                      <p className="text-sm font-black text-brand-ink">Inspection evidence needs review</p>
                      <p className="mt-1 text-[11px] leading-5 text-zinc-500">One tread reading is old, one is estimated and one position has no physical measurement. Schedule a verified inspection before relying on replacement decisions.</p>
                    </div>
                ...
                <section className="rounded-[22px] bg-white p-5 ring-1 ring-black/[0.045]">
                  <div className="flex items-center justify-between">
                    <div><h3 className="text-sm font-black text-brand-ink">Mileage history</h3><p className="mt-1 text-[11px] text-zinc-400">Latest captured readings</p></div>
                    ...
                    {selectedHistory.slice(0, 3).map((reading, index) => {
                      ...
                      return (
                        <div key={reading.id} className="flex gap-3 py-3 first:pt-0 last:pb-0">
                          <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${sourceStyles[reading.source]}`}><HistorySourceIcon size={15} /></div>
